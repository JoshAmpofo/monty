...
void push(stack_t **stack, unsigned int line_number)
{
	...
	if (new == NULL)
	{
		optoken_err(malloc_error());
		return;
	}

	if (op_toks[1] == NULL)
	{
		optoken_err(intnum_err(line_number));
		return;
	}
...
void pint(stack_t **stack, unsigned int line_number)
{
	if ((*stack)->next == NULL)
	{
		optoken_err(pint_err(line_number));
		return;
	}
...
void pop(stack_t **stack, unsigned int line_number)
{
	...
	if ((*stack)->next == NULL)
	{
		optoken_err(pop_err(line_number));
		return;
	}
...
void swap(stack_t **stack, unsigned int line_number)
{
	...
	{
		optoken_err(short_stack_err(line_number, "swap"));
		return;
	}
